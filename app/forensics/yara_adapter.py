...
class YaraAdapter:
    ...
    def scan(
        ...
        *,
        ...
        # -m emits rule metadata and -s emits matching strings. YARA returns 0
        # whether or not a file matched; parser therefore treats empty stdout as
        # a valid zero-match result.
    ...
    def _parse_output(output: str) -> list[dict[str, Any]]:
        ...
        Rule lines begin with the rule token. String-detail lines generated by
        `-s` begin with an offset (for example `0x10:$a:`). Metadata emitted by
        `-m` varies slightly across YARA builds, so unknown detail lines are
        retained as raw strings instead of guessed into security meaning.
        """
        ...
        for raw in output.splitlines():
            ...
            # Standard form is: RULE [tags] target-path. We only need the rule
            # name reliably; tags are best-effort and all remaining text is kept.
            parts = line.split()
            if not parts:
                continue
            current = {
