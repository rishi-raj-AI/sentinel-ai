...
def graph_neighbors(case_id: str, query: str, depth: int = 1, max_results: int = 100):
    case_dir = CaseManager().root / case_id
    CaseManager().load_case(case_id)
    return EvidenceGraphQueryEngine(case_dir).neighbors(query, depth=depth, max_results=max_results)


def graph_trace(case_id: str, source: str, target: str, max_hops: int = 6):
    case_dir = CaseManager().root / case_id
    CaseManager().load_case(case_id)
    return EvidenceGraphQueryEngine(case_dir).trace(source, target, max_hops=max_hops)


def attack_mapping(case_id: str, max_candidates: int = 25):
    case_dir = CaseManager().root / case_id
    CaseManager().load_case(case_id)
...
def investigate_case(case_id: str, window_seconds: int = 120, max_findings: int = 25):
    case_dir = CaseManager().root / case_id
    CaseManager().load_case(case_id)
    result = InvestigationEngine(case_dir).analyze(window_seconds=window_seconds, max_findings=max_findings)
    network = NetworkIntelligenceEngine(case_dir).analyze(max_flows=max_findings)
    result["findings"] = [f for f in result.get("findings", []) if not _is_packet_level_network_finding(f)]

    aggregated_findings: list[dict] = []
    for flow in network["findings"]["public_destination_flows"]:
        aggregated_findings.append({"title": "Public destination network flow", "severity": "medium", "confidence": "medium", "reason": f"{flow.get('src')}:{flow.get('src_port')} -> {flow.get('dst')}:{flow.get('dst_port')}", "flow": flow})
    for flow in network["findings"]["unusual_destination_ports"]:
        aggregated_findings.append({"title": "Unusual destination port", "severity": "medium", "confidence": "medium", "reason": f"Network flow used destination port {flow.get('dst_port')}", "flow": flow})
...
def collect_macos_logs(case_id: str, last: str = "5m", predicate: str | None = None, max_events: int = 1000):
    case_dir = CaseManager().root / case_id
    CaseManager().load_case(case_id)
    return MacOSLogAdapter(case_dir).collect(last=last, predicate=predicate, max_events=max_events)
...
def run_volatility(case_id: str, image_path: str, plugin: str, evidence_id: str | None = None):
    case_dir = CaseManager().root / case_id
    CaseManager().load_case(case_id)
    return VolatilityAdapter(case_dir).run(image_path=image_path, plugin=plugin, evidence_id=evidence_id)
...
def run_volatility_evidence(case_id: str, evidence_id: str, plugin: str):
    ...
    if not stored_path.is_file():
        raise FileNotFoundError(f"Stored evidence file not found: {stored_path}")
    case_dir = CaseManager().root / case_id
    result = VolatilityAdapter(case_dir).run(image_path=str(stored_path), plugin=plugin, evidence_id=evidence_id)
    result.update({"case_id": case_id, "evidence_id": evidence_id, "evidence_sha256_verified": True, "chain_of_custody_valid": True})
    return result
...
def analyze_pcap_evidence(case_id: str, evidence_id: str, max_packets: int = 5000):
    ...
    if not stored_path.is_file():
        raise FileNotFoundError(f"Stored evidence file not found: {stored_path}")
    case_dir = CaseManager().root / case_id
    result = TsharkAdapter(case_dir).analyze(pcap_path=str(stored_path), evidence_id=evidence_id, max_packets=max_packets)
    result.update({"case_id": case_id, "evidence_id": evidence_id, "evidence_sha256_verified": True, "chain_of_custody_valid": True})
    return result
