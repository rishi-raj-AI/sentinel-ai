...
class DashboardService:
    ...
    def __init__(self, cases_root: str = "cases", sigma_rules: str = "rules/sigma") -> None:
    ...
    def chat(self, case_id: str, question: str) -> dict[str, Any]:
        ...
        if not q:
            return {"answer": "Ask a question about the current case.", "sources": []}
        brief = CaseReasoningEngine(case_dir, sigma_rules=self.sigma_rules).build(max_items=25)
        sources: list[str] = ["case_brief"]

        if any(word in q for word in ("summary", "status", "assessment", "what happened", "case")):
            answer = f"{brief['headline']}. Confidence: {brief['confidence']}. "
            if brief.get("hypotheses"):
                answer += brief["hypotheses"][0].get("statement", "")
            return {"answer": answer, "sources": sources}

        if any(word in q for word in ("next", "recommend", "action", "gap", "missing")):
            actions = brief.get("recommended_actions", [])
            gaps = brief.get("collection_gaps", [])
            answer = "Recommended next actions: " + ("; ".join(actions[:6]) if actions else "No additional actions are currently listed.")
            if gaps:
                answer += " Collection gaps: " + "; ".join(gaps[:5])
            return {"answer": answer, "sources": sources}

        if any(word in q for word in ("sigma", "detection", "alert", "yara")):
            det = self.detections(case_id, max_items=25)
            sources.append("detections")
            sigma_rows = det["sigma"].get("detections", [])
            yara_rows = det["yara"]
            answer = f"Sigma detections: {len(sigma_rows)}. YARA unique matches: {len(yara_rows)}."
            if sigma_rows:
                answer += " Sigma: " + "; ".join(f"{x.get('title')} ({x.get('level')}, {x.get('evidence_id')})" for x in sigma_rows[:5]) + "."
            if yara_rows:
                answer += " YARA: " + "; ".join(f"{x.get('rule')} on {x.get('evidence_id')} x{x.get('match_count')}" for x in yara_rows[:5]) + "."
            return {"answer": answer, "sources": sources}

        if any(word in q for word in ("mitre", "attack", "technique")):
            attack = AttackMappingEngine(case_dir).analyze(max_candidates=25)
            sources.append("attack_mapping")
            rows = attack.get("candidates", [])
            if not rows:
                return {"answer": "No ATT&CK candidates are currently supported by the ingested evidence.", "sources": sources}
            return {"answer": "ATT&CK candidates: " + "; ".join(f"{r.get('technique_id')} {r.get('technique_name')} ({r.get('confidence')})" for r in rows), "sources": sources}

        if any(word in q for word in ("network", "ip", "port", "dns", "flow")) or re.search(r"\b\d{1,3}(?:\.\d{1,3}){3}\b", q):
            network = NetworkIntelligenceEngine(case_dir).analyze(max_flows=50)
            sources.append("network_intelligence")
            unusual = network.get("findings", {}).get("unusual_destination_ports", [])
            if unusual:
                text = "; ".join(f"{f.get('protocol') or 'network'} {f.get('src')}:{f.get('src_port')} -> {f.get('dst')}:{f.get('dst_port')}" for f in unusual[:8])
                return {"answer": f"Unusual network flows: {text}.", "sources": sources}
            return {"answer": f"The case contains {network.get('flow_count', 0)} aggregated network flows and no unusual destination-port finding.", "sources": sources}

        if any(word in q for word in ("evidence", "hash", "file")):
            record = self.manager.load_case(case_id)
            rows = record.get("evidence", [])
            sources.append("case_evidence")
            text = "; ".join(f"{r.get('evidence_id')} {r.get('filename')}" for r in rows[:10])
            return {"answer": f"Registered evidence items: {len(rows)}. {text}", "sources": sources}

        observations = brief.get("observations", [])
        if observations:
            answer = "The strongest current observations are: " + "; ".join(f"{x.get('title')}: {x.get('basis')}" for x in observations[:5])
        else:
            answer = "No strong observations are currently listed. Ask about evidence, detections, network activity, ATT&CK, or recommended actions."
        return {"answer": answer, "sources": sources}


def create_dashboard_app(cases_root: str = "cases", sigma_rules: str = "rules/sigma") -> FastAPI:
    service = DashboardService(cases_root=cases_root, sigma_rules=sigma_rules)
    ...
    def health() -> dict[str, Any]:
        return {"status": "ok", "mode": "local-read-only-dashboard"}
...
#graphSvg{width:100%;height:390px;background:#0b1520;border-radius:9px;border:1px solid var(--line)}.chatlog{height:285px;overflow:auto;padding:4px;display:flex;flex-direction:column;gap:8px}.msg{max-width:87%;padding:9px 11px;border-radius:10px;line-height:1.4;font-size:13px}.user{align-self:flex-end;background:#173a4d}.assistant{align-self:flex-start;background:#152532;border:1px solid #264052}.chatbox{display:flex;gap:8px;margin-top:10px}.chatbox input{flex:1}.small{font-size:12px;color:var(--muted)}a{color:#7dd3fc;text-decoration:none}a:hover{text-decoration:underline}.empty{color:var(--muted);font-size:13px}.loading{opacity:.55;pointer-events:none}
...
<div class="card chat"><h2>Investigator Assistant</h2><div class="small">Read-only answers grounded in the currently ingested case data.</div><div class="chatlog" id="chatLog"><div class="msg assistant">Ask about the case summary, detections, evidence, network activity, ATT&CK, or recommended next actions.</div></div><form class="chatbox" id="chatForm"><input id="chatInput" autocomplete="off" placeholder="What are the strongest findings?"><button>Ask</button></form></div>
</div></main>
<script>
