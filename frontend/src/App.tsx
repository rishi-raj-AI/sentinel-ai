...
function missionAgentRows(workspace?:MissionWorkspace):[string,string,number,string][]{
  const rows=workspace?.review?.stage_reviews||[]
  if(!rows.length) return agents
  return rows.slice(0,6).map(r=>{
    const score=r.assessment==='supported'?94:r.assessment==='contested'?58:r.state==='completed'?82:66
    const tone=r.assessment==='contested'?'amber':r.state==='completed'?'green':'cyan'
    return [r.agent,`${r.stage_id} · ${r.assessment}`,score,tone]
  })
}

function missionReasoning(workspace?:MissionWorkspace):[string,string,string][]{
  const frames=workspace?.replay?.frames||[]
  if(!frames.length) return reasoningRows
  return frames.slice(-5).map(f=>[
    f.at?new Date(f.at).toLocaleTimeString([], {hour:'2-digit',minute:'2-digit'}):'LIVE',
    f.label,
    f.state==='failed'?'red':f.state==='completed'?'green':f.state==='running'?'cyan':'violet',
  ])
}

function CommandCenter({data,onSelectCase,workspace}:{data:CommandData;onSelectCase:(c:CaseSummary)=>void;workspace?:MissionWorkspace}){
  ...
  const twinLabels=workspace?.twin?.nodes?.filter(n=>n.node_id!=='MISSION').map(n=>n.label).slice(0,8)||[]
  const activeAgents=missionAgentRows(workspace)
  const liveReasoning=missionReasoning(workspace)
  return <div className="page-transition">
    <IntelligenceTicker items={workspace?[`Active mission ${workspace.mission_id}`,`${workspace.evidence_workspace?.count||0} evidence records`,`${confidence}% supervisor confidence`,`${workspace.memory?.related?.length||0} related missions`,...intelItems]:intelItems}/>
    <div className="hero-2">
      <div><div className="eyebrow">SENTINEL X / GLOBAL SECURITY FABRIC</div><h1>Cyber Command Center</h1><p>{workspace?`Active mission ${workspace.mission_id}: ${workspace.snapshot?.mission?.objective||'mission intelligence synchronized'}`:'Evidence-grounded autonomous security operations across investigations, code, identity, cloud and simulated infrastructure.'}</p></div>
      <div className="core-stage"><ReactorXL online={healthy}/><div className="core-stage-copy"><strong>{healthy?'SENTINEL CORE ONLINE':'CORE DEGRADED'}</strong><span>{workspace?`${workspace.mission_id} · ${workspace.snapshot?.mission?.status||'ACTIVE'}`:`${data.status.generation||'X12'} · API ${data.status.backend_api_version||'1.0'}`}</span><i>{workspace?'MISSION INTELLIGENCE / SYNCHRONIZED':'AI FABRIC / NOMINAL'}</i></div></div>
    </div>
    <div className="metrics-grid">{workspace?<><Metric label="MISSION STATE" value={(workspace.snapshot?.mission?.status||'—').toUpperCase()} sub={workspace.mission_id}/><Metric label="EVIDENCE" value={workspace.evidence_workspace?.count||0} sub={`${workspace.evidence_workspace?.facets?.counter_evidence||0} counter-evidence`} tone="violet"/><Metric label="SUPERVISOR" value={`${confidence}%`} sub={supervisor?.verdict||'awaiting review'} tone="amber"/><Metric label="RELATED" value={workspace.memory?.related?.length||0} sub="historical mission matches" tone="green"/></>:<><Metric label="ACTIVE CASES" value={data.cases.length} sub="investigation workspaces"/><Metric label="DATA STORES" value={storeCount||7} sub="persistent intelligence layers" tone="violet"/><Metric label="CONTROL TENANTS" value={data.control.tenant_count??0} sub={`${data.control.audit_decisions??0} policy decisions`} tone="amber"/><Metric label="TWIN COVERAGE" value={`${Math.round((data.twin.coverage_ratio??0)*100)}%`} sub="modeled control coverage" tone="green"/></>}</div>
    <div className="command-grid ui2-grid">
      <Panel title={workspace?'Active Mission Digital Twin':'Operational Digital Twin'} kicker="X8 / PREDICTIVE DEFENSE" className="span-2 cinematic" action={<Pill tone={workspace?'green':'violet'}>{workspace?'MISSION SYNC':'LIVE MODEL'}</Pill>}><FabricMap labels={twinLabels.length?twinLabels:fabricLabels}/></Panel>
      <Panel title="Sentinel Core / Agent Wall" kicker="MULTI-AGENT ORCHESTRATION" className="agent-panel" action={<Activity size={16} className="cyan"/>}><AgentWall rows={activeAgents}/></Panel>
      <Panel title="Global Signal Field" kicker="INTELLIGENCE RADAR" action={<Pill tone="cyan">LAB VIEW</Pill>}><GlobeField primary={Math.max(1,workspace?.evidence_workspace?.count||4)} secondary={(workspace?.memory?.related?.length??data.insights.insight_count??0)+11}/></Panel>
      <Panel title="Investigation Queue" kicker="EVIDENCE-FIRST CASES"><div className="case-list">{data.cases.slice(0,6).map((c,i)=><button className="case-row" key={c.case_id} onClick={()=>onSelectCase(c)}><span className={`severity ${i===0?'sev-high':'sev-med'}`}/><span className="case-main"><strong>{c.case_id}</strong><small>{c.title||'Security investigation'}</small></span><span className="case-meta">{c.evidence_count??0}<small>evidence</small></span><ChevronRight size={15}/></button>)}{!data.cases.length&&<div className="empty-state"><Aperture size={26}/><span>{workspace?'Mission intelligence active; no case queue items returned.':'No active cases detected'}</span></div>}</div></Panel>
      <Panel title="Live Radar" kicker="OPERATIONS / TELEMETRY"><RadarField/></Panel>
      <Panel title="AI Reasoning Timeline" kicker="EXPLAINABLE DECISIONS"><ReasoningTimeline rows={liveReasoning}/></Panel>
...
function LabView({view,data,workspace}:{view:View;data:CommandData;workspace?:MissionWorkspace}){
  ...
  const twinLabels=workspace?.twin?.nodes?.filter(n=>n.node_id!=='MISSION').map(n=>n.label).slice(0,8)||[]
  const activeAgents=missionAgentRows(workspace)
  return <div className="page-transition module-page"><div className="module-hero"><div className="module-icon"><Icon size={30}/></div><div><div className="eyebrow">{kicker}</div><h1>{title}</h1><p>{workspace&&special==='twin'?`Mission ${workspace.mission_id} projected over the infrastructure model.`:subtitle}</p></div></div>
    ...
    {special==='twin'&&<div className="module-grid"><Panel title={workspace?'Active Mission Fabric':'Infrastructure Fabric'} kicker="PREDICTIVE MODEL" className="span-2 cinematic" action={workspace?<Pill tone="green">{workspace.mission_id}</Pill>:undefined}><FabricMap labels={twinLabels.length?twinLabels:fabricLabels}/></Panel><Panel title="Mission Context" kicker="INTELLIGENCE OVERLAY"><div className="status-stack"><div><Activity/><span>Mission state</span><Pill tone="cyan">{workspace?.snapshot?.mission?.status||'NO MISSION'}</Pill></div><div><Database/><span>Evidence</span><Pill tone="violet">{workspace?.evidence_workspace?.count||0}</Pill></div><div><ShieldCheck/><span>Supervisor</span><Pill tone="green">{Math.round((workspace?.snapshot?.supervisor?.confidence||0)*100)}%</Pill></div></div></Panel><Panel title="Coverage Radar" kicker="CONTROL SURFACE"><RadarField/></Panel></div>}
    {special==='soc'&&<div className="module-grid"><Panel title="Agent Wall" kicker="COORDINATED ANALYSIS"><AgentWall rows={activeAgents}/></Panel><Panel title="Decision Timeline" kicker="SUPERVISOR"><ReasoningTimeline rows={missionReasoning(workspace)}/></Panel><Panel title="Operational Fabric" kicker="LIVE MODEL" className="span-2"><FabricMap labels={twinLabels.length?twinLabels:fabricLabels}/></Panel></div>}
    {special==='intel'&&<div className="module-grid"><Panel title="Global Intelligence Field" kicker="X12 / SHARED SIGNALS"><GlobeField primary={Math.max(1,workspace?.evidence_workspace?.count??data.insights.insight_count??1)} secondary={(workspace?.memory?.related?.length||0)+18}/></Panel><Panel title="Signal Radar" kicker="CORRELATION"><RadarField/></Panel><Panel title="Intelligence Stream" kicker="LIVE FEED" className="span-2"><IntelligenceTicker items={workspace?[`Mission ${workspace.mission_id} synchronized`,`${workspace.memory?.related?.length||0} historical matches`,...intelItems]:intelItems}/></Panel></div>}
    {special==='research'&&<div className="module-grid"><Panel title="Evaluation Matrix" kicker="QUALITY LAB"><div className="research-score"><strong>{data.evaluation.result_count??0}</strong><span>recorded runs</span><div className="score-ring">{Math.round((workspace?.review?.quality_score||.95)*100)}<small>%</small></div></div></Panel><Panel title="Reasoning Audit" kicker="REPRODUCIBILITY"><ReasoningTimeline rows={missionReasoning(workspace)}/></Panel><Panel title="Benchmark Fabric" kicker="SYSTEM COMPARISON" className="span-2"><AgentWall rows={activeAgents.map((x,i)=>[x[0],`mission/review lane ${i+1}`,Math.max(50,x[2]-3),'cyan'])}/></Panel></div>}
    ...
    {special==='generic'&&<div className="module-grid"><Panel title="System Surface" kicker="LIVE CAPABILITIES"><div className="phase-grid">{Object.entries(data.status.phases||{}).map(([k,v])=><div className="phase-chip" key={k}><CircleDot size={12}/><strong>{k}</strong><span>{v}</span></div>)}</div></Panel><Panel title="Operational State" kicker="BACKEND TELEMETRY"><div className="status-stack"><div><ServerCog/><span>Backend API</span><Pill tone="green">ONLINE</Pill></div><div><Database/><span>Persistent stores</span><Pill tone="cyan">READY</Pill></div><div><BrainCircuit/><span>Agent fabric</span><Pill tone="violet">ACTIVE</Pill></div></div></Panel><Panel title="Interactive Fabric" kicker="LAB WORKSPACE" className="span-2"><FabricMap labels={twinLabels.length?twinLabels:fabricLabels}/></Panel></div>}
  </div>
}

export default function App(){
  const [view,setView]=useState<View>('command');const [data,setData]=useState<CommandData>(empty);const [loading,setLoading]=useState(true);const [selected,setSelected]=useState<CaseSummary|undefined>();const [overview,setOverview]=useState<any>(null);const [menu,setMenu]=useState(false);const [multi,setMulti]=useState(false);const [workspace,setWorkspace]=useState<MissionWorkspace|undefined>()
  ...
  useEffect(()=>{
    let alive=true
    const refreshMission=async()=>{const id=activeMissionId();if(!id)return;try{const value=await loadMissionWorkspace(id);if(alive)setWorkspace(value)}catch{}}
    const onMission=(event:Event)=>{const detail=(event as CustomEvent<MissionWorkspace>).detail;if(detail)setWorkspace(detail)}
    refreshMission();window.addEventListener(MISSION_EVENT,onMission)
    const id=window.setInterval(refreshMission,5000)
    return()=>{alive=false;window.removeEventListener(MISSION_EVENT,onMission);window.clearInterval(id)}
  },[])
  ...
  const clearMission=()=>{try{localStorage.removeItem(ACTIVE_MISSION_KEY)}catch{};setWorkspace(undefined)}
  return <div className={`app-shell ${multi?'multi-mode':''}`}><div className="ambient ambient-one"/><div className="ambient ambient-two"/><div className="scanlines"/>
    <aside className={`sidebar ${menu?'sidebar-open':''}`}><div className="brand"><div className="brand-mark"><ShieldCheck size={18}/></div><div><strong>SENTINEL <b>X</b></strong><span>CYBER OPERATIONS LAB</span></div><button className="sidebar-close" onClick={()=>setMenu(false)}><X size={16}/></button></div><div className="nav-section-label">OPERATIONS</div><nav>{nav.map(({id,label,icon:Icon})=><button key={id} className={view===id?'active':''} onClick={()=>{setView(id);setMenu(false)}}><Icon size={16}/><span>{label}</span>{view===id&&<i/>}</button>)}</nav><div className="sidebar-foot"><div className="core-mini"><div className={`core-dot ${healthy?'ok':'bad'}`}/><div><strong>SENTINEL CORE</strong><span>{healthy?'Systems nominal':'Backend degraded'}</span></div></div><div className="build">UI 3.2 · {workspace?`${workspace.mission_id} ACTIVE`:'BACKEND API 1.0'}</div></div></aside>
    <main className="main-shell"><header className="topbar"><button className="mobile-menu" onClick={()=>setMenu(true)}><Menu size={17}/></button><div className="breadcrumb"><span>SENTINEL X</span><ChevronRight size={11}/><strong>{title.toUpperCase()}</strong>{workspace&&<><ChevronRight size={11}/><button className="active-mission-chip" onClick={clearMission}>{workspace.mission_id} ×</button></>}</div><div className="top-search"><Search size={15}/><input placeholder="Search cases, IOCs, assets, evidence..."/></div><div className="top-actions"><Pill tone={healthy?'green':'red'}>{healthy?'SYSTEM ONLINE':'DEGRADED'}</Pill><button className={loading?'spin':''} onClick={refresh}><RefreshCw size={15}/></button><button onClick={()=>setMulti(!multi)} className={multi?'active-tool':''}><Maximize2 size={15}/></button><button><Bell size={15}/><span className="notification-dot"/></button><div className="operator"><span>RR</span><div><strong>OPERATOR</strong><small>Lead Analyst</small></div></div></div></header>
      <div className="content">{view==='command'?<CommandCenter data={data} onSelectCase={openCase} workspace={workspace}/>:view==='cases'?<InvestigationLab selected={selected} overview={overview}/>:view==='tools'?<ToolManagerLab/>:<LabView view={view} data={data} workspace={workspace}/>}</div>
      <div className="telemetry-footer"><span><i className={workspace?'ok':healthy?'ok':''}/> {workspace?`MISSION ${workspace.mission_id}`:`CORE ${healthy?'ONLINE':'DEGRADED'}`}</span><span>API {data.status.backend_api_version||'1.0'}</span><span>{data.status.generation||'X12'}</span><span>{workspace?`EVIDENCE ${workspace.evidence_workspace?.count||0}`:`CASES ${data.cases.length}`}</span><span>{workspace?`CONF ${Math.round((workspace.snapshot?.supervisor?.confidence||0)*100)}%`:`TWIN ${Math.round((data.twin.coverage_ratio??0)*100)}%`}</span><span className="footer-live"><Activity size={10}/> LIVE TELEMETRY</span></div>
    </main>
  </div>
}
